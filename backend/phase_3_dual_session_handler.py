...
class DualSessionHandler:
    ...
    async def crawl_with_dual_sessions(self, session_a_token: str, session_b_token: str, endpoints: list[str]) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            for endpoint in endpoints:
                try:
                    ...
                    response = await client.get(f'{self.base_url}{endpoint}', headers=headers, follow_redirects=True)
    ...
    async def _extract_resources(self, html_content: str, endpoint: str) -> list[dict[str, Any]]:
        ...
        json_ids = re.findall(r'"(?:id|userId|resourceId|itemId)"\s*:\s*(?:"?(\d+|[a-f0-9\-]+)"?)', html_content)
