...
class SemanticFuzzer:
    ...
    def __init__(
        ...
        self.application_indicators = {
            ApplicationType.BANKING: {
                'keywords': ['bank', 'transaction', 'account', 'payment', 'transfer', 'deposit', 'withdraw', 'balance', 'credit', 'debit', 'loan', 'mortgage', 'interest', 'routing', 'swift', 'iban', 'ach'],
                'endpoints': ['/api/transfer', '/api/payment', '/api/account', '/api/transaction', '/api/balance', '/api/deposit', '/api/withdraw'],
                'forms': ['account_number', 'routing_number', 'amount', 'recipient', 'transfer_type', 'payment_method'],
                'parameters': ['account_id', 'transaction_id', 'payment_id', 'transfer_id', 'balance', 'amount', 'currency', 'recipient_id']
            },
            ApplicationType.ECOMMERCE: {
                'keywords': ['shop', 'cart', 'product', 'order', 'purchase', 'checkout', 'inventory', 'shipping', 'billing', 'coupon', 'discount', 'wishlist', 'review', 'catalog'],
                'endpoints': ['/api/cart', '/api/order', '/api/product', '/api/checkout', '/api/payment', '/api/inventory'],
                'forms': ['product_id', 'quantity', 'price', 'discount_code', 'shipping_address', 'billing_address', 'payment_method'],
                'parameters': ['product_id', 'order_id', 'cart_id', 'item_id', 'sku', 'quantity', 'price', 'discount_code', 'coupon_code']
            },
            ApplicationType.HEALTHCARE: {
                'keywords': ['patient', 'doctor', 'medical', 'health', 'diagnosis', 'treatment', 'prescription', 'record', 'appointment', 'hospital', 'clinic', 'insurance'],
                'endpoints': ['/api/patient', '/api/record', '/api/appointment', '/api/prescription', '/api/diagnosis'],
                'forms': ['patient_id', 'medical_record', 'diagnosis_code', 'prescription_id', 'appointment_date', 'insurance_number'],
                'parameters': ['patient_id', 'record_id', 'appointment_id', 'prescription_id', 'diagnosis_id', 'insurance_number', 'medical_code']
            },
            ApplicationType.SOCIAL_MEDIA: {
                'keywords': ['user', 'profile', 'post', 'comment', 'like', 'share', 'follow', 'friend', 'message', 'chat', 'group', 'community', 'timeline'],
                'endpoints': ['/api/user', '/api/profile', '/api/post', '/api/comment', '/api/message', '/api/friend'],
                'forms': ['username', 'email', 'password', 'profile_picture', 'bio', 'post_content', 'comment_text'],
                'parameters': ['user_id', 'post_id', 'comment_id', 'message_id', 'group_id', 'profile_id', 'friend_id']
            },
            ApplicationType.ENTERPRISE: {
                'keywords': ['employee', 'hr', 'payroll', 'timesheet', 'project', 'task', 'report', 'dashboard', 'analytics', 'workflow', 'approval', 'document'],
                'endpoints': ['/api/employee', '/api/project', '/api/task', '/api/report', '/api/approval'],
                'forms': ['employee_id', 'project_id', 'task_id', 'report_id', 'approval_id', 'document_id'],
                'parameters': ['employee_id', 'project_id', 'task_id', 'report_id', 'approval_id', 'department_id', 'manager_id']
            }
        }
    ...
    def _generate_value_patterns(self, app_type: ApplicationType) -> List[str]:
        ...
        patterns = {
            ApplicationType.BANKING: [
                '123456789', '987654321', '1000000', '0.01', '999999.99',
                'USD', 'EUR', 'GBP', 'CREDIT', 'DEBIT', 'TRANSFER',
                'CHECKING', 'SAVINGS', 'LOAN', 'MORTGAGE'
            ],
            ApplicationType.ECOMMERCE: [
                '999999', '1', '0', '0.01', '99.99', 'FREE',
                'DISCOUNT', 'SALE', 'PROMO', 'COUPON',
                'SHIPPING', 'TAX', 'INVENTORY', 'STOCK'
            ],
            ApplicationType.HEALTHCARE: [
                '123456789', 'MRN', 'DOB', 'ICD10', 'CPT',
                'PATIENT', 'DOCTOR', 'NURSE', 'HOSPITAL',
                'CLINIC', 'INSURANCE', 'PRESCRIPTION'
            ],
            ApplicationType.SOCIAL_MEDIA: [
                'user123', 'testuser', 'admin', 'moderator',
                'public', 'private', 'friends', 'followers',
                'likes', 'shares', 'comments', 'posts'
            ],
            ApplicationType.ENTERPRISE: [
                'EMP001', 'MGR001', 'DEPT001', 'PROJ001',
    ...
    async def _ai_enhance_wordlist(self, wordlist: ContextualWordlist) -> Dict[str, Any]:
        ...
        return {'additional_terms': [], 'additional_parameters': [], 'additional_patterns': [], 'additional_form_fields': []}
    ...
    def _compile_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'summary': {
                ...
                'average_confidence': sum(wl.confidence for wl in self.contextual_wordlists) / max(len(self.contextual_wordlists), 1),
