...
class MassScanner:
        ...
        try:
            ...
            for result in results:
                if isinstance(result, Exception):
                    ...
                    target = next((t for t in self.targets if t.domain == result.target_domain), None)
                    if target:
    ...
    async def _scan_target_with_semaphore(self, semaphore: asyncio.Semaphore, target: MassScanTarget, template: TemplateValidation) -> ScanResult:
    ...
    async def _scan_single_target(self, target: MassScanTarget, template: TemplateValidation) -> ScanResult:
        ...
        try:
            ...
            cmd = [
                ...
                '-u', f'https://{target.domain}',
                '-json',
                '-silent',
                '-timeout', str(self.scan_timeout),
                '-rate-limit', str(int(1 / self.rate_limit_delay))
            ]
        ...
        return {
            ...
            'summary': {
                ...
                'high_severity_findings': len(self.consolidated_results.get('by_severity', {}).get('critical', [])) + 
                                        len(self.consolidated_results.get('by_severity', {}).get('high', [])),
                'success_rate': len([t for t in self.targets if t.status == ScanStatus.COMPLETED]) / max(len(self.targets), 1),
