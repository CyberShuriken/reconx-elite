...
async def demo_ssrf_missed_detection():
    ...
    verification_data = {
        ...
        "additional_context": {
            ...
            "required_headers": ["X-Request-ID: manual-test", "Content-Type: application/json"],
...
async def show_learning_summary():
    ...
    if learning_data['verification_results']:
        ...
        for i, result in enumerate(learning_data['verification_results'], 1):
            print(f"  {i}. {result['vulnerability_id']} - {result['verification_status']} (Confidence: {result['original_confidence']} -> {result['adjusted_confidence']})")
