...
class AIRouter:
    ...
    async def call_model(
        ...
        if model_config['provider'] == 'openrouter' and not self.openrouter_key:
            return {'role': role, 'model': model_config['model_id'], 'output': '', 'error': 'missing_openrouter_key'}
        
        if model_config['provider'] == 'gemini' and not self.gemini_key:
            return {'role': role, 'model': model_config['model_id'], 'output': '', 'error': 'missing_gemini_key'}
        ...
        if model_config['provider'] == 'gemini':
            ...
            payload = self._prepare_openrouter_payload(prompt, max_tokens, model_config['temperature'], model_config['model_id'])
    ...
    def _prepare_gemini_payload(self, prompt: str, max_tokens: int, temperature: float) -> dict[str, Any]:
        ...
        return {
            'contents': [{
                'parts': [{
                    'text': f"You are a security AI assistant.\n\n{prompt}"
                }]
            }],
            'generationConfig': {
    ...
    def _prepare_openrouter_payload(self, prompt: str, max_tokens: int, temperature: float, model_id: str) -> dict[str, Any]:
    ...
    async def classify_hosts(self, hosts_list: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for host in hosts_list:
            ...
            category = 'api_target' if '/api' in name or 'api.' in name else 'auth_target' if 'login' in name or 'admin' in name else 'dev_target' if 'dev' in name else 'admin_target' if 'admin' in name else 'skip'
    ...
    async def analyze_headers(self, headers_dict: dict[str, str], target: str) -> list[dict[str, Any]]:
        findings = []
        if headers_dict.get('access-control-allow-origin') in ('*', 'https://evil-reconx.com'):
        ...
        if 'content-security-policy' not in headers_dict:
            findings.append({'issue': 'Missing Content-Security-Policy header', 'severity': 'Medium'})
        return findings

    async def rate_severity(self, finding_dict: dict[str, Any]) -> dict[str, Any]:
        severity = finding_dict.get('severity', 'Low')
        if severity.lower() == 'critical':
            return {'cvss_score': 9.8, 'cvss_vector': 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H'}
        if severity.lower() == 'high':
            return {'cvss_score': 7.5, 'cvss_vector': 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:N'}
        if severity.lower() == 'medium':
            return {'cvss_score': 5.4, 'cvss_vector': 'CVSS:3.1/AV:N/AC:L/PR:N/UI:R/S:U/C:L/I:L/A:N'}
        return {'cvss_score': 3.1, 'cvss_vector': 'CVSS:3.1/AV:N/AC:H/PR:N/UI:R/S:U/C:L/I:N/A:N'}

    async def find_chains(self, findings_list: list[dict[str, Any]]) -> list[dict[str, Any]]:
        if len(findings_list) < 2:
            return []
        return [{'chain_id': 1, 'findings': [f.get('id') for f in findings_list], 'severity': 'High', 'description': 'Potential attack chain connecting multiple findings.'}]
    ...
    async def analyze_target_recon(self, target: str, discovered_info: dict[str, Any]) -> dict[str, Any]:
        return {'priority_targets': [target], 'attack_plan': 'Focus on live hosts, API endpoints, and exposed metadata.'}
    ...
    async def consensus_debate_mode(self, vulnerability_data: Dict[str, Any], severity: str, config: Optional[DebateModeConfig] = None) -> ConsensusResult:
    ...
    async def _secondary_model_review(self, vulnerability_data: Dict[str, Any], primary_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _calculate_consensus(self, primary_result: Dict[str, Any], secondary_result: Dict[str, Any], config: DebateModeConfig) -> ConsensusResult:
