...
DATABASE_URL = os.getenv('DATABASE_URL') or f"sqlite+aiosqlite:///{os.getenv('DB_PATH', './reconx.db')}"
...
class Finding(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    session_id = Column(String, ForeignKey('scans.session_id'), nullable=False, index=True)
