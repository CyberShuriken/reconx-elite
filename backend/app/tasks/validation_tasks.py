...
async def validate_vulnerability_task(vulnerability_id: int, payload: str = None) -> dict:
    ...
    try:
        ...
        existing_validation = db.query(ExploitValidation).filter(
        ...
        logger.info(f"Validation completed for vulnerability {vulnerability_id}: {validation_result.get('validation_status')}")
