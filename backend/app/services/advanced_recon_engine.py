...
class StealthScanner:
    ...
    def _calculate_delay(self) -> float:
        ...
        base_delay = random.uniform(
            self.config.random_delay_min / 1000.0,
            self.config.random_delay_max / 1000.0
        )
...
class ParameterDiscovery:
    ...
    def __init__(self):
        self.common_params = [
            ...
            "id", "user", "username", "email", "token", "key", "secret",
            "redirect", "url", "return", "next", "callback", "ref",
            "file", "upload", "download", "path", "dir", "folder",
            "action", "method", "type", "format", "view", "mode",
            "page", "limit", "offset", "sort", "order", "filter",
            "search", "query", "q", "term", "keyword", "find",
            "lang", "language", "locale", "country", "region",
            "debug", "test", "dev", "staging", "admin", "config",
            # API parameters
            "api_key", "apikey", "access_token", "refresh_token",
            "client_id", "client_secret", "grant_type", "scope",
            "response_type", "state", "nonce", "code",
            # Security parameters
            "csrf_token", "authenticity_token", "samesite",
            "captcha", "challenge", "verification", "otp",
            # Technical parameters
            "version", "v", "build", "revision", "timestamp",
        ...
        for param in self.common_params:
            try:
                ...
                result = await self._test_parameter(
                    base_url, param, "query", stealth_scanner
                )
                if result:
                ...
                result = await self._test_parameter(
                    base_url, param, "post", stealth_scanner
                )
                if result:
                    discovered_params.append(result)
    ...
    def __init__(self):
        self.common_wordlists = {
            "admin": [
                ...
                "backup", "backup.php", "backup.sql", "dump",
                "test", "dev", "staging", "debug", "debug.php",
            ],
            "api": [
                "api", "v1", "v2", "v3", "rest", "graphql",
                "endpoint", "service", "microservice", "health",
                "status", "ping", "version", "info", "docs",
                "swagger", "openapi", "redoc", "postman",
            ],
            "files": [
                "robots.txt", "sitemap.xml", "sitemap.html",
                ".htaccess", ".htpasswd", "web.config",
                "index.php", "index.html", "index.jsp", "index.asp",
                "config.php", "config.inc", "config.cfg",
                "database.yml", "database.ini", "env", ".env",
                "log", "logs", "error.log", "access.log",
                "upload", "uploads", "files", "download",
                "tmp", "temp", "cache", "storage",
            ],
            "directories": [
                "admin", "api", "assets", "css", "js", "img", "images",
                "uploads", "files", "backup", "logs", "tmp", "cache",
                "config", "conf", "etc", "lib", "vendor", "node_modules",
                "src", "app", "public", "private", "secure", "hidden",
            ]
        }
