...
class CustomTemplateEngine:
    ...
    def create_template(
        ...
        try:
            ...
            if severity not in self.valid_severities:
                return False, f"Invalid severity. Must be one of: {self.valid_severities}", None
    ...
    def update_template(
        ...
        try:
            template = db.query(CustomNucleiTemplate).filter(
    ...
    def run_template(
        ...
        try:
            ...
            return True, f"Template executed successfully. Found {len(stored_results)} matches.", findings
    ...
    def get_template_results(
        ...
        return db.query(CustomTemplateResult).filter(
            CustomTemplateResult.template_id == template_id
        ).order_by(CustomTemplateResult.created_at.desc()).limit(limit).all()
    ...
    def get_public_templates(
        ...
        return db.query(CustomNucleiTemplate).filter(
            CustomNucleiTemplate.is_public == True,
            CustomNucleiTemplate.is_active == True,
            CustomNucleiTemplate.is_valid == True
        ).order_by(CustomNucleiTemplate.successful_detections.desc()).limit(limit).all()
    ...
    def search_templates(
        ...
        user_templates = db.query(CustomNucleiTemplate).filter(
            CustomNucleiTemplate.user_id == user_id,
            CustomNucleiTemplate.is_active == True
        ).filter(
        ...
        if include_public:
            public_templates = db.query(CustomNucleiTemplate).filter(
                CustomNucleiTemplate.is_public == True,
                CustomNucleiTemplate.is_active == True
            ).filter(
                (CustomNucleiTemplate.name.ilike(search_pattern)) |
