...
async def get_learning_statistics(
    ...
    high_confidence_patterns = db.query(LearningPattern).filter(
        LearningPattern.user_id == current_user.id,
        LearningPattern.confidence_score >= 80
    ).count()
    
    # Payload statistics
    total_payloads = db.query(SuccessfulPayload).filter(
        SuccessfulPayload.user_id == current_user.id
    ).count()
    
    high_success_payloads = db.query(SuccessfulPayload).filter(
        SuccessfulPayload.user_id == current_user.id,
        SuccessfulPayload.success_rate >= 80
    ).count()
    
    # Endpoint statistics
    total_endpoints = db.query(HighValueEndpoint).filter(
        HighValueEndpoint.user_id == current_user.id
    ).count()
    
    high_priority_endpoints = db.query(HighValueEndpoint).filter(
        HighValueEndpoint.user_id == current_user.id,
...
async def delete_successful_payload(
    ...
    payload = db.query(SuccessfulPayload).filter(
