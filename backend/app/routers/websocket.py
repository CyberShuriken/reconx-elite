...
async def scan_websocket(
    ...
    # Verify scan exists and is owned by user via target.owner_id
    result = await db.execute(
        select(Scan)
        .join(Target, Target.id == Scan.target_id)
        .filter(and_(Scan.id == scan_id, Target.owner_id == user_id))
    )
    ...
    await log_audit_event(
        db,
        action="scan_websocket_connected",
        user_id=user_id,
        ip_address=websocket.client.host if websocket.client else None,
        metadata_json={"scan_id": scan_id, "user_agent": websocket.headers.get("user-agent", "unknown")},
    )
    try:
        while True:
            data = await websocket.receive_text()
                    ...
                    json.dumps(
                        {
                            ...
                            "data": {"scan_id": scan_id, "message": "Subscribed to scan stream"},
        ...
        logger.error("Scan WebSocket error for scan %s: %s", scan_id, e)
        manager.disconnect_scan(websocket, scan_id)
    finally:
        await log_audit_event(
            db,
            action="scan_websocket_disconnected",
            user_id=user_id,
            ip_address=websocket.client.host if websocket.client else None,
