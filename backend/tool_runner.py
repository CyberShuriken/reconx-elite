...
class ToolRunner:
    ...
    async def check_sensitive_paths(self, base_url: str) -> list[dict[str, Any]]:
        paths = [
            ...
            '/api/swagger.json', '/actuator', '/actuator/env', '/phpinfo.php',
            '/server-status', '/debug', '/.DS_Store', '/robots.txt', '/sitemap.xml',
        ]
    ...
    async def _fetch_path(self, client: httpx.AsyncClient, url: str) -> dict[str, Any] | None:
        try:
            ...
            return {'path': url, 'status_code': response.status_code, 'content_length': len(response.content)}
        except Exception:
            return None
    ...
    async def check_subdomain_takeover(self, subdomain: str) -> dict[str, Any]:
        try:
            ...
            return {'vulnerable': False, 'cname': None, 'service': None, 'error': str(exc)}
        vulnerable_services = [
            'github.io', 'herokuapp.com', 's3.amazonaws.com', 'azurewebsites.net',
            'fastly.net', 'shopify.com', 'zendesk.com', 'readme.io', 'ghost.io',
        ]
