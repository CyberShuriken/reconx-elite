...
class AgenticBrowser:
    ...
    def __init__(
        ...
        self.auth_patterns = {
            ...
            AuthenticationType.FORM_LOGIN: {
                ...
                'selectors': ['input[type="password"]', 'form[action*="login"]', 'form[action*="auth"]']
            },
    ...
    async def _handle_request(self, route) -> None:
        ...
        if '/api/' in request.url:
            ...
            # Avoid duplicates
            if not any(r.route_path == route_info.route_path and r.route_method == route_info.route_method 
        ...
        try:
            ...
            for form in login_forms:
                ...
                if any(indicator in form_action.lower() for indicator in ['login', 'auth', 'signin']):
                    ...
                    username_field = await form.query_selector('input[type="email"], input[type="text"], input[name*="user"], input[name*="email"], input[id*="user"]')
    ...
    async def _extract_session_data(self) -> None:
        ...
        try:
            ...
            local_storage = await self.page.evaluate('() => Object.keys(localStorage).reduce((obj, key) => { obj[key] = localStorage[key]; return obj; }, {})')
            self.browser_session.local_storage = local_storage
            
            # Extract session storage
            session_storage = await self.page.evaluate('() => Object.keys(sessionStorage).reduce((obj, key) => { obj[key] = sessionStorage[key]; return obj; }, {})')
    ...
    def _compile_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'browser_session': {
                'authentication_type': self.browser_session.authentication_type.value if self.browser_session else 'unknown',
