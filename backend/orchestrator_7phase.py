...
from backend.phase_4_5_tactical_strikes import AdvancedInjectionOrchestrator, TacticalStrikesOrchestrator
...
class SevenPhaseOrchestrator:
    ...
    async def phase_1_recursive_recon(self) -> None:
        ...
        await self.ws_manager.send_log(self.session_id, 'info', 'Starting Phase 1: Recursive Recon', phase='phase_1')
        ...
        try:
            ...
            self.manifest.update_phase('phase_1', 'completed', [{
    ...
    async def phase_2_context_aware_profiling(self) -> None:
        ...
        await self.ws_manager.send_log(self.session_id, 'info', 'Starting Phase 2: Tech Profiling & CVE Analysis', phase='phase_2')
        
        try:
            live_hosts = self.manifest.data['context_tree'].get('live_hosts', [])
            if not live_hosts:
                live_hosts = await self.tool_runner.run_httpx_urls(self.manifest.data['context_tree'].get('subdomains', []))
            ...
            self.manifest.add_context('api_endpoints', api_surface.get('rest', []) + api_surface.get('graphql', []))
            
            self.manifest.update_phase('phase_2', 'completed', [{
    ...
    async def phase_3_authenticated_analysis(self) -> None:
        ...
        await self.ws_manager.send_log(self.session_id, 'info', 'Starting Phase 3: Authenticated Analysis', phase='phase_3')
        
        try:
            if not self.session_tokens.get('session_a') or not self.session_tokens.get('session_b'):
                await self.ws_manager.send_log(
                    self.session_id, 'warning',
                    'Phase 3 skipped: No session tokens provided', phase='phase_3'
                )
    ...
    async def phase_4_vulnerability_strikes(self) -> None:
        ...
        await self.ws_manager.send_log(self.session_id, 'info', 'Starting Phase 4: Tactical Strikes', phase='phase_4')
        ...
        try:
            ...
            idor_findings = await orchestrator.execute_bac_idor_strikes(endpoints, self.session_tokens.get('session_a', ''))
            ...
            self.manifest.update_phase('phase_4', 'completed', {
    ...
    async def phase_5_advanced_injection(self) -> None:
        ...
        await self.ws_manager.send_log(self.session_id, 'info', 'Starting Phase 5: Advanced Injection', phase='phase_5')
        ...
        try:
            ...
            self.manifest.update_phase('phase_5', 'completed', {
    ...
    async def phase_7_reporting(self) -> None:
        ...
        await self.ws_manager.send_log(self.session_id, 'info', 'Starting Phase 7: Report Generation', phase='phase_7')
