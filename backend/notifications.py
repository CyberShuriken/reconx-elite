...
class NotificationHub:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Failed to send {notification_msg.platform.value} notification: {response.status_code}")
    ...
    async def notify_new_subdomain(self, subdomain: str, target: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        ...
        return await self.send_notification(
            NotificationType.NEW_SUBDOMAIN,
            title,
            message,
            "info",
            metadata
    ...
    async def notify_monitoring_error(self, error_message: str, target: str, error_type: str = "general") -> bool:
        ...
        return await self.send_notification(
            NotificationType.MONITORING_ERROR,
            title,
            message,
            "high",
            metadata
    ...
    async def notify_scan_completed(self, target: str, total_vulnerabilities: int, critical_count: int, high_count: int, duration: str) -> bool:
    ...
    async def notify_critical_vulnerability(self, vulnerability_id: str, severity: str, endpoint: str, description: str) -> bool:
    ...
    async def notify_mass_scan_completed(self, targets_count: int, templates_count: int, total_findings: int, duration: str) -> bool:
        ...
        return await self.send_notification(
            NotificationType.MASS_SCAN_COMPLETED,
            title,
            message,
            severity,
            metadata
    ...
    async def notify_consensus_alert(self, vulnerability_id: str, consensus_score: float, determination: str, reasoning: str) -> bool:
        ...
        return await self.send_notification(
            NotificationType.CONSENSUS_ALERT,
            title,
            message,
            severity,
            metadata
