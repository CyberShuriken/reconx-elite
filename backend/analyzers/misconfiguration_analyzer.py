...
class JSSecretScanner:
    ...
    SECRET_PATTERNS = [
        ...
        (r'(?:aws[_-]?access[_-]?key|aws[_-]?key)["\']?\s*[:=]\s*["\']([A-Z0-9]{20})', 'AWS Access Key'),
        (r'(?:aws[_-]?secret|aws[_-]?secret[_-]?key)["\']?\s*[:=]\s*["\']([a-zA-Z0-9/+=]{40})', 'AWS Secret Key'),
        (r'(?:github[_-]?token|gh[_-]?token)["\']?\s*[:=]\s*["\'](ghp_[a-zA-Z0-9]{36})', 'GitHub Token'),
        (r'(?:stripe[_-]?key|stripe[_-]?secret)["\']?\s*[:=]\s*["\'](sk_live_[a-zA-Z0-9]{24,})', 'Stripe Key'),
        (r'(?:google[_-]?api[_-]?key|gcp[_-]?key)["\']?\s*[:=]\s*["\'](AIza[a-zA-Z0-9_-]{35})', 'Google API Key'),
        (r'(?:password|passwd|pwd)["\']?\s*[:=]\s*["\']([^"\']{8,})', 'Password'),
        (r'(?:secret|private[_-]?key)["\']?\s*[:=]\s*["\']([a-zA-Z0-9]{20,})', 'Secret/Private Key'),
        (r'-----BEGIN (?:RSA )?PRIVATE KEY-----', 'Private Key'),
        (r'(?:bearer|token)["\']?\s*[:=]\s*["\'](eyJ[a-zA-Z0-9_-]{10,}\.[a-zA-Z0-9_-]{10,})', 'JWT Token'),
    ]
...
class ExposedFileChecker:
    ...
    SENSITIVE_FILES = [
        ...
        '/config.json', '/package.json', '/docker-compose.yml',
        '/wp-config.php', '/.htaccess', '/server-status',
        '/phpinfo.php', '/debug/pprof', '/actuator/env',
        '/api/swagger.json', '/graphql/playground',
    ]
...
async def analyze_misconfiguration(
    ...
    vulnerable_services = [
        ...
        'azurewebsites.net', 'fastly.net', 'shopify.com',
        'zendesk.com', 'readme.io', 'ghost.io', 'cloudfront.net',
    ]
