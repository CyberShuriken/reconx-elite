            ...
            async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
                ...
                if payload in response_text or 'alert(1)' in response_text.lower():
                    ...
                    if any(block_indicator in response_text.lower() for block_indicator in [
                        'blocked', 'forbidden', 'firewall', 'security', 'not acceptable'
                    ]):
                        return {'blocked': True, 'payload': payload, 'response': response_text[:200]}
                    else:
...
async def analyze_xss(
    ...
    reflection_params = [
        'q', 'search', 'query', 'keyword', 'message', 'content',
        'text', 'comment', 'name', 'title', 'description', 'email', 'phone',
    ]
