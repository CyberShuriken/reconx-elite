...
class JWTTester:
    ...
    async def test_weak_secret(token: str) -> dict[str, Any]:
        ...
        try:
            ...
            weak_secrets = ['secret', 'password', '123456', 'key', 'jwt_secret', 'reconx']
...
async def analyze_auth_session(
    ...
    findings = {'jwt_endpoints': [], 'oauth_endpoints': [], 'weak_auth': [], 'jwt_vulnerabilities': []}
    ...
    if 'jwt' in tech_str and token:
        jwt_endpoints = [
            ...
            if 'login' in e.get('path', '').lower() or 'auth' in e.get('path', '').lower()
        ]
