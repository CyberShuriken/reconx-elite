        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
                ...
                if any(indicator in response_text for indicator in [
                    'ami-id', 'instance-id', 'mac',  # AWS
                    'compute', 'metadata',  # GCP/Azure
                    'PONG', 'redis_version',  # Redis
                    'kubernetes', 'kubelet',  # K8s
...
async def analyze_ssrf(
    ...
    url_like_params = [
        ...
        'redirect_url', 'return_url', 'next', 'image_url', 'file_url',
        'source', 'target', 'fetch', 'load', 'proxy',
    ]
    ...
    return {
        ...
        'recommendation': 'Target cloud metadata endpoints for credential extraction'
        if is_cloud
