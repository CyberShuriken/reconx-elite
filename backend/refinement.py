...
class Refinement:
    ...
    async def _generate_few_shot_examples(self) -> None:
        ...
        confirmed_results = [r for r in self.verification_results if r.verification_status == VerificationStatus.CONFIRMED]
    ...
    async def _create_learning_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'false_positive_patterns': [
                entry.pattern for entry in recent_entries 
    ...
    def _compile_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'verification_results': {
                ...
                'confirmed_count': len([r for r in self.verification_results if r.verification_status == VerificationStatus.CONFIRMED]),
                'false_positive_count': len([r for r in self.verification_results if r.verification_status == VerificationStatus.FALSE_POSITIVE]),
                'partial_count': len([r for r in self.verification_results if r.verification_status == VerificationStatus.PARTIAL]),
                'average_confidence_adjustment': sum(r.confidence_adjustment for r in self.verification_results) / max(len(self.verification_results), 1)
            },
