...
class TacticalStrikesOrchestrator:
    ...
    async def execute_bac_idor_strikes(self, endpoints: list[dict[str, Any]], session_token: str = '') -> list[dict[str, Any]]:
    ...
    async def execute_ssrf_strikes(self, endpoints: list[dict[str, Any]], tech_stack: dict[str, str]) -> list[dict[str, Any]]:
    ...
    async def execute_business_logic_strikes(self, endpoints: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        payment_keywords = ['checkout', 'payment', 'cart', 'order', 'purchase', 'invoice', 'price']
...
class AdvancedInjectionOrchestrator:
    ...
    async def execute_graphql_strikes(self, graphql_endpoints: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for endpoint in graphql_endpoints:
            if endpoint.get('type') == 'GraphQL':
                ...
                introspection_result = await self.ai_router.call_model(self.model_role, introspection_prompt, max_tokens=500)
    ...
    async def execute_prompt_injection_strikes(self, endpoints: list[dict[str, Any]], tech_stack: dict[str, str]) -> list[dict[str, Any]]:
        """Execute prompt injection attacks against LLM/Chatbot endpoints."""
        findings = []
        
        # Detect LLM/Chatbot endpoints
        llm_keywords = ['chat', 'ai', 'chatbot', 'gpt', 'llm', 'assistant', 'ask', 'query']
        
        for endpoint in endpoints:
            endpoint_path = endpoint.get('path', '').lower()
            if any(keyword in endpoint_path for keyword in llm_keywords) or any(keyword in str(tech_stack).lower() for keyword in llm_keywords):
    ...
    async def execute_injection_strikes(self, endpoints: list[dict[str, Any]], tech_stack: dict[str, str]) -> list[dict[str, Any]]:
