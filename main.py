...
class ReconXEliteEngine:
    ...
    async def _update_session_tokens_from_browser(self, browser_results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            for cookie in cookies:
                if cookie.get('name') in ['sessionid', 'session', 'auth', 'token']:
                    self.session_tokens[f"cookie_{cookie['name']}"] = cookie.get('value', '')
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        successful_tests = [t for t in sandbox_tests if t.get('test_result', '').startswith('success')]
...
async def main():
    ...
    # Check arguments
    if len(sys.argv) < 2:
        print("Usage: python main.py <target> [options] [session_token_a] [session_token_b]")
        print("Options:")
        ...
        print("Example: python main.py example.com --mass-scan templates/*.yaml --domain-filter example.com")
        sys.exit(1)
    ...
    while i < len(sys.argv):
        ...
        if arg == '--monitor':
            ...
            if i + 1 < len(sys.argv):
                ...
                print("Error: --domain-filter requires a value")
                sys.exit(1)
        else:
            # Session token
            break
    ...
    session_tokens = {}
    if i < len(sys.argv):
        session_tokens['session_a'] = sys.argv[i]
    if i + 1 < len(sys.argv):
        session_tokens['session_b'] = sys.argv[i + 1]
    ...
    try:
        if mass_scan_mode:
            ...
            if not template_paths:
                print("Error: --mass-scan requires at least one template path")
                sys.exit(1)
