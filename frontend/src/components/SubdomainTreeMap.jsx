...
const SubdomainTreeMap = ({ subdomains, vulnerabilities, endpoints }) => {
  const treeData = useMemo(() => {
    ...
    const environmentGroups = subdomains.reduce((acc, subdomain) => {
      ...
      const subdomainVulns = vulnerabilities.filter(
        (v) => v.matched_url && v.matched_url.includes(subdomain.hostname),
      ).length;

      const subdomainEndpoints = endpoints.filter((e) => e.hostname === subdomain.hostname).length;

      const size = Math.max(
        100,
        subdomainVulns * 50 + subdomainEndpoints * 20 + (subdomain.is_live ? 100 : 20),
      );
      ...
      if (data.children) {
        ...
        return (
          <div className="custom-tooltip">
            ...
            <p>
              <strong>Subdomains:</strong> {data.children.length}
            </p>
            <p>
              <strong>Total Vulnerabilities:</strong> {data.totalVulns}
            </p>
            <p>
              <strong>Total Endpoints:</strong> {data.totalEndpoints}
            </p>
            <p>
              <strong>Risk Level:</strong>
              <span className={`risk-indicator risk-${data.riskLevel}`}>{data.riskLevel}</span>
            </p>
          </div>
        );
      } else {
        ...
        return (
          <div className="custom-tooltip">
            ...
            <p>
              <strong>Vulnerabilities:</strong> {data.vulnerabilities}
            </p>
            <p>
              <strong>Endpoints:</strong> {data.endpoints}
            </p>
            {data.takeover_candidate && (
              <p>
                <strong>⚠️ Takeover Candidate</strong>
              </p>
            )}
            {data.cdn && (
              <p>
                <strong>CDN:</strong> {data.cdn}
              </p>
            )}
            {data.waf && (
              <p>
                <strong>WAF:</strong> {data.waf}
              </p>
            )}
            {data.tags.length > 0 && (
              <p>
      ...
      <div className="treemap-stats">
        ...
        {treeData.children.map((env) => (
          <div key={env.name} className="env-stat">
            <h5>
              {env.name}
              <span className={`risk-indicator risk-${env.riskLevel}`}>{env.riskLevel} risk</span>
            </h5>
