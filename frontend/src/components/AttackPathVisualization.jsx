  ...
  const graphData = useMemo(() => {
    ...
    endpoints.forEach((endpoint, index) => {
      ...
      const node = {
        id,
        name:
          endpoint.normalized_url.length > 50
      ...
      const subdomainNode = Array.from(nodeMap.entries()).find(
        ([key]) => key.includes("subdomain") && subdomains.find((s) => s.hostname === hostname),
      );
          ...
          {hoverNode.data && (
            <>
              {hoverNode.type === "vulnerability" && (
                <>
                  <p>
                    <strong>Severity:</strong> {hoverNode.data.severity}
                  </p>
                  <p>
                    <strong>Confidence:</strong> {hoverNode.data.confidence}
                  </p>
                  <p>
                    <strong>Description:</strong> {hoverNode.data.description}
                  </p>
                </>
              )}
              {hoverNode.type === "endpoint" && (
                <>
                  <p>
                    <strong>Priority Score:</strong> {hoverNode.data.priority_score}
                  </p>
                  <p>
                    <strong>Category:</strong> {hoverNode.data.category}
                  </p>
                  <p>
                    <strong>Tags:</strong> {(hoverNode.data.tags || []).join(", ")}
                  </p>
                </>
              )}
              {hoverNode.type === "subdomain" && (
                <>
                  <p>
                    <strong>Live:</strong> {hoverNode.data.is_live ? "Yes" : "No"}
                  </p>
                  <p>
                    <strong>Environment:</strong> {hoverNode.data.environment}
                  </p>
                  <p>
                    <strong>Takeover Candidate:</strong>{" "}
                    {hoverNode.data.takeover_candidate ? "Yes" : "No"}
                  </p>
                </>
              )}
            </>
      ...
      {selectedPath && (
        <div className="selected-path-panel">
          <div className="panel-header">
            ...
            <button onClick={() => setSelectedPath(null)} className="close-button">
              ×
            </button>
          </div>
          <div className="panel-content">
            {selectedPath.template_id ? (
              ...
              <div>
                ...
                <p>
                  <strong>Confidence:</strong> {selectedPath.confidence}
                </p>
                <p>
                  <strong>Matched URL:</strong> {selectedPath.matched_url}
                </p>
                <p>
                  <strong>Description:</strong> {selectedPath.description}
                </p>
              ...
              <div>
                ...
                <p>
                  <strong>URL:</strong> {selectedPath.url}
                </p>
                <p>
                  <strong>Priority Score:</strong> {selectedPath.priority_score}
                </p>
                <p>
                  <strong>Category:</strong> {selectedPath.category}
                </p>
                <p>
                  <strong>Source:</strong> {selectedPath.source}
                </p>
                <p>
                  <strong>Tags:</strong> {(selectedPath.tags || []).join(", ")}
                </p>
                <p>
                  <strong>Focus Reasons:</strong> {(selectedPath.focus_reasons || []).join(", ")}
                </p>
              </div>
            )}
          </div>
