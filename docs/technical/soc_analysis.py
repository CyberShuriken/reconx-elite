...
# Ordered severity levels for consistent sorting
SEVERITY_ORDER = ['p1', 'critical', 'p2', 'high', 'p3', 'medium', 'p4', 'low', 'p5', 'info']
...
class SOCAnalyzer:
    ...
    def _parse_notification_line(self, line: str) -> Optional[Dict[str, Any]]:
        """Parse a single notification line with fallback handling"""
        try:
            ...
            if not timestamp_match:
                logger.debug(f"Could not parse timestamp from line: {line[:50]}")
                return None
            ...
            if 'NEW_SUBDOMAIN' in message_upper:
                notification_type = 'new_subdomain'
            elif 'VALID_POC' in message_upper:
                notification_type = 'valid_poc'
            elif 'CONSENSUS_ALERT' in message_upper:
                notification_type = 'consensus_alert'
            elif 'NUCLEI_TEMPLATE' in message_upper:
                notification_type = 'nuclei_template'
            elif 'JS_CHANGE' in message_upper:
                notification_type = 'js_change'
            elif 'ATTACK_CHAIN' in message_upper:
    ...
    def _analyze_vulnerabilities(self) -> None:
        ...
        if not isinstance(findings, list):
            logger.warning("Phase 4 findings is not a list, skipping vulnerability analysis")
            return
        ...
        for finding in findings:
            ...
            if finding.get('type') in ['sql_injection', 'xss_reflected', 'xss_stored', 'idor', 'ssrf', 'rce', 'lfi', 'business_logic']:
                vuln_finding = VulnerabilityFinding(
                    ...
                    endpoint=finding.get('data', {}).get('endpoint', '') if isinstance(finding.get('data'), dict) else '',
                    method=finding.get('data', {}).get('method', '') if isinstance(finding.get('data'), dict) else '',
                    payload=finding.get('data', {}).get('payload', '') if isinstance(finding.get('data'), dict) else '',
                    evidence=finding.get('data', {}).get('evidence', '') if isinstance(finding.get('data'), dict) else '',
    ...
    def _analyze_consensus_results(self) -> None:
        ...
        if not isinstance(consensus_data, list):
            logger.warning("Consensus analysis data is not a list")
            return
        ...
        for consensus in consensus_data:
            ...
            analysis = ConsensusAnalysis(
                ...
                primary_confidence=float(consensus.get('primary_confidence', 0.0)),
                secondary_confidence=float(consensus.get('secondary_confidence', 0.0)),
                consensus_score=float(consensus.get('consensus_score', 0.0)),
                agreement=bool(consensus.get('agreement', False)),
                flaws_detected=consensus.get('flaws_detected', []) if isinstance(consensus.get('flaws_detected'), list) else [],
    ...
    def _generate_consensus_recommendation(self, consensus: Dict[str, Any]) -> str:
        ...
        # Ensure flaws is a list for processing
        if not isinstance(flaws, list):
            flaws = []
        
        # Convert flaws to lowercase strings for comparison
        flaws_str = ' '.join(str(f).lower() for f in flaws)
        ...
        if determination == 'pass' and severity in ['P1', 'critical']:
            ...
            if 'false positive' in flaws_str:
    ...
    def _analyze_template_status(self) -> None:
        ...
        if not isinstance(templates_data, list):
            logger.warning("Nuclei templates data is not a list")
            return
        ...
        for template in templates_data:
            ...
            status = TemplateStatus(
                ...
                validation_errors=template.get('validation_errors', []) if isinstance(template.get('validation_errors'), list) else [],
                template_info=template.get('template_info', {}) if isinstance(template.get('template_info'), dict) else {}
            )
    ...
    def _analyze_deltas(self) -> None:
        ...
        # Initialize deltas with safe defaults
        if not isinstance(deltas, dict):
            logger.warning(f"Delta data has unexpected type: {type(deltas).__name__}, using defaults")
            deltas = {
                'new_subdomains': [],
                'js_changes': [],
                'ssl_changes': [],
        ...
        # Ensure all expected keys are lists
        for key in ['new_subdomains', 'js_changes', 'ssl_changes', 'header_changes', 'technology_changes']:
            if key not in deltas or not isinstance(deltas[key], list):
                deltas[key] = []
    ...
    def _extract_subdomain_details(self) -> List[Dict[str, Any]]:
        ...
        for finding in findings:
            if isinstance(finding, dict) and finding.get('type') == 'new_subdomain':
    ...
    def _extract_js_change_details(self) -> List[Dict[str, Any]]:
        ...
        for finding in findings:
            if isinstance(finding, dict) and finding.get('type') == 'js_file_change':
                data = finding.get('data', {})
                if isinstance(data, dict):
                    js_changes.append(data)
    ...
    def _assess_delta_risk(self, deltas: Dict[str, Any]) -> str:
        ...
        new_subdomains = deltas.get('new_subdomains', [])
        if isinstance(new_subdomains, list):
            risk_score += len(new_subdomains) * 2
        
        # JS changes with sensitive data are high risk
        js_changes = deltas.get('js_changes', [])
        if isinstance(js_changes, list):
        ...
        tech_changes = deltas.get('technology_changes', [])
        if isinstance(tech_changes, list):
            risk_score += len(tech_changes) * 3
    ...
    def _calculate_exploitability(self) -> None:
        ...
        for vuln in self.vulnerabilities:
            ...
            # Avoid division by zero - if confidence is 0, use 0.1 floor
            adjusted_score = base_score * max(0.1, confidence)
    ...
    def _generate_verification_command(self, vuln: VulnerabilityFinding) -> str:
        ...
        endpoint = vuln.endpoint or 'http://target/endpoint'
        method = vuln.method or 'GET'
        payload = vuln.payload or 'test_payload'
        
        if vuln.type == 'sql_injection':
            return f"curl -X '{method}' '{endpoint}' -H 'Content-Type: application/json' -d '{{\"user_id\":\"{payload}\"}}'"
        elif vuln.type == 'xss_reflected':
            return f"curl -X '{method}' '{endpoint}?q={payload}'"
        elif vuln.type in ['idor', 'ssrf', 'lfi']:
            return f"curl -X '{method}' '{endpoint}' -d '{payload}'"
        else:
            return f"curl -X '{method}' '{endpoint}'"
    ...
    def _rank_by_exploitability(self) -> List[Dict[str, Any]]:
        ...
        sorted_vulns = sorted(
            ...
            key=lambda x: exploitability_order.get(x.exploitability, 5)
        )
    ...
    def _rank_by_severity(self) -> List[Dict[str, Any]]:
        """Rank vulnerabilities by severity using ordered list"""
        def severity_sort_key(vuln: VulnerabilityFinding) -> int:
            severity_lower = vuln.severity.lower()
            try:
                return SEVERITY_ORDER.index(severity_lower)
            except ValueError:
                # Unknown severity - place at end
                return len(SEVERITY_ORDER)
        
        sorted_vulns = sorted(self.vulnerabilities, key=severity_sort_key)
    ...
    def _rank_by_business_impact(self) -> List[Dict[str, Any]]:
        ...
        def impact_sort_key(vuln: VulnerabilityFinding) -> int:
            impact_prefix = vuln.business_impact.split(':')[0].strip()
            return impact_order.get(impact_prefix, 4)
        
        sorted_vulns = sorted(self.vulnerabilities, key=impact_sort_key)
